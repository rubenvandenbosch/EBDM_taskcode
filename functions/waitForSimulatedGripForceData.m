   function [squeezeData, keys, timeOfLastAcquisition] = waitForSimulatedGripForceData(ex,...
...
while(GetSecs < startRecordingTime + maxTimeToWait && ~EXIT)              % how do I do this for right and left hand
   ...
   if(any(keys==KbName('UpArrow'))), force = force + 50; end
   if(any(keys==KbName('DownArrow'))), force = force - 50; end
   force = max(force,1);
   force = min(1000,force);
   
   % get current acquisition status (where are we, how many samples have
   % been acquired up to now)
   %newhdr = ft_read_header(url);
   newhdr.nSamples = ceil(ex.MP_SAMPLE_RATE * (GetSecs-startRecordingTime));
   ...
   if(newhdr.nSamples > oldhdr.nSamples)                 % any data in queue?
      ...
      nChans = 2;
      data = rand(nChans,newhdr.nSamples-oldhdr.nSamples) .* force;
      lastread=GetSecs;
